...
ESEventAction::ESEventAction() : G4UserEventAction(), fHitsCollectionID(-1)
{
...
ESHitsCollection *ESEventAction::GetHitsCollection(G4int hcID,
                                                   const G4Event *event) const
{
  ESHitsCollection *hitsCollection =
      static_cast<ESHitsCollection *>(event->GetHCofThisEvent()->GetHC(hcID));

  if (!hitsCollection) {
    G4ExceptionDescription msg;
    msg << "Cannot access hitsCollection ID " << hcID;
    // check how to use G4Exception
    G4Exception("ESEventAction::GetHitsCollection()", "MyCode0003",
                FatalException, msg);
  }
