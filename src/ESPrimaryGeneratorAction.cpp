...
ESPrimaryGeneratorAction::ESPrimaryGeneratorAction(BeamState beam,
                                                   G4double beamEne)
      ...
      fMessenger(nullptr)
{
  ...
  if (fBeamState == BeamState::Wide || fBeamState == BeamState::MonoWide)
    fThetaMax = 3.5 * mrad;
  // fThetaMax = atan((kMagnetGap / 2.) / -kSourceZPos);
  else
    fThetaMax = 1.5 * mrad;  // zero angle and reference are set later
  ...
  if (fBeamState == BeamState::Mono || fBeamState == BeamState::MonoZero ||
      fBeamState == BeamState::MonoWide)
    GunPointer = &ESPrimaryGeneratorAction::MonoEneGun;
  else if (fBeamState == BeamState::Reference)
    GunPointer = &ESPrimaryGeneratorAction::ReferenceGun;
  else
    GunPointer = &ESPrimaryGeneratorAction::UniformGun;
  ...
  if (fBeamState == BeamState::ZeroAng || fBeamState == BeamState::MonoZero)
    ...
    AngGenPointer = &ESPrimaryGeneratorAction::UniformAng;
...
void ESPrimaryGeneratorAction::ReferenceGun() { fBeamEne += 10. * MeV; }
...
{
  fMessenger = new G4GenericMessenger(this, "/ES/Primary/", "Beam control");

  // z position
  G4GenericMessenger::Command &sourceZCmd = fMessenger->DeclareMethodWithUnit(
      "Z", "mm", &ESPrimaryGeneratorAction::SetSourceZ,
